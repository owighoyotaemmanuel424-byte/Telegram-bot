...
export default defineSchema({
  ...
  jobs: defineTable({ userId: v.id('users'), conversationId: v.optional(v.id('conversations')), status: jobStatus, prompt: v.string(), intent: v.string(), provider: v.optional(v.string()), providerJobId: v.optional(v.string()), progress: v.number(), creditsReserved: v.number(), outputUrl: v.optional(v.string()), error: v.optional(v.string()), createdAt: v.number(), updatedAt: v.number() }).index('by_user', ['userId']).index('by_status', ['status']),
