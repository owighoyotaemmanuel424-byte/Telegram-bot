...
export default defineSchema({
  ...
  jobs: defineTable({ userId: v.id('users'), conversationId: v.optional(v.id('conversations')), telegramChatId: v.optional(v.string()), telegramStatusMessageId: v.optional(v.number()), status: jobStatus, prompt: v.string(), intent: v.string(), provider: v.optional(v.string()), providerJobId: v.optional(v.string()), progress: v.number(), creditsReserved: v.number(), sourceAssetIds: v.optional(v.array(v.id('mediaAssets'))), outputUrl: v.optional(v.string()), outputAssets: v.optional(v.any()), error: v.optional(v.string()), createdAt: v.number(), updatedAt: v.number() }).index('by_user', ['userId']).index('by_status', ['status']),
