    ...
    try {
      ...
      if (this.mediaIngestion && assets.length) {
        ...
        for (const asset of stored) {
          const id = await this.convex.createMediaAsset({ userId, telegramFileId: asset.telegramFileId, storageKey: asset.key, type: asset.type, mimeType: asset.mimeType, conversationId });
          await this.convex.setActiveAsset(conversationId, userId, id);
