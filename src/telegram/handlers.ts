...
type BuildState = { status: 'queued' | 'building' | 'deploying' | 'completed' | 'failed'; message: string; updatedAt: number; branch?: string; projectName?: string; prompt?: string; url?: string; adminEmail?: string; adminPassword?: string };
...
export class TelegramHandlers {
  ...
  async handle(update: TelegramUpdate): Promise<void> {
    ...
    if (input === '/start' || input === '/menu') { await this.api.sendMessage(chatId, '🤖 AI App Builder + Media Assistant\n\n/build <description> — generate, validate and deploy an app automatically\n/deploy <branch> — deploy an existing generated branch\n/status — show your latest build/deployment'); return; }
    if (input === '/help') { await this.api.sendMessage(chatId, '🏗️ /build <description> — generate → validate → repair → deploy\n🚀 /deploy <branch> — deploy an existing branch\n📊 /status — latest build/deployment status\n\nIf your app request includes admin/authentication, the bot provisions credentials through Vercel environment variables and returns them after the live health check.'); return; }
    if (input === '/status') { const build = this.builds.get(telegramId); await this.api.sendMessage(chatId, build ? `🏗️ Status: ${build.status}\n\n${build.message}${build.branch ? `\n🌿 ${build.branch}` : ''}${build.projectName ? `\n📱 ${build.projectName}` : ''}${build.url ? `\n🌐 ${build.url}` : ''}${build.adminEmail ? `\n\n🔐 Admin email: ${build.adminEmail}\n🔑 Admin password: ${build.adminPassword}` : ''}` : 'No app build has been started yet. Use /build <what you want to build>.'); return; }
  ...
  private async handleBuild(chatId: string, telegramId: string, prompt: string): Promise<void> {
    ...
    if (!this.appBuilder || !process.env.GITHUB_TOKEN || !process.env.GITHUB_OWNER || !process.env.GITHUB_REPOSITORY || !process.env.VERCEL_TOKEN) { await this.api.sendMessage(chatId, '⚠️ Autonomous App Builder is not configured. Set GitHub, Gemini and Vercel settings on the server.'); return; }
    this.builds.set(telegramId, { status: 'building', message: 'Generating and validating project…', updatedAt: Date.now(), prompt }); const status = await this.api.sendMessage(chatId, '🏗️ AUTONOMOUS BUILD STARTED\n\n1/5 🧠 Generate\n2/5 🐙 GitHub\n3/5 🔍 Build + AI repair\n4/5 🚀 Deploy to Vercel\n5/5 ❤️ Live health check'); const statusMessageId = (status as { message_id: number }).message_id;
    try {
      ...
      this.builds.set(telegramId, { status: 'deploying', message: `${result.summary}\n📦 ${result.files.length} files\nDeploying production…`, updatedAt: Date.now(), branch: result.branch, projectName: result.projectName, prompt });
      await this.api.editMessage(chatId, statusMessageId, `🔍 BUILD PASSED\n\n📱 ${result.projectName}\n📦 ${result.files.length} files\n🌿 ${result.branch}\n\n🚀 Deploying to Vercel…`);
      const deployment = await this.appBuilder.deployExistingBranch(process.env.GITHUB_OWNER, process.env.GITHUB_REPOSITORY, result.branch, result.projectName, prompt);
      const adminEmail = deployment.adminCredentials?.email; const adminPassword = deployment.adminCredentials?.password;
      this.builds.set(telegramId, { status: 'completed', message: `🌐 ${deployment.url}\n❤️ Health check: HTTP ${deployment.health.status}`, updatedAt: Date.now(), branch: result.branch, projectName: result.projectName, prompt, url: deployment.url, adminEmail, adminPassword });
      const admin = adminEmail && adminPassword ? `\n\n🔐 ADMIN LOGIN\nEmail: ${adminEmail}\nPassword: ${adminPassword}\n\n⚠️ Save these credentials securely.` : '';
      await this.api.editMessage(chatId, statusMessageId, `🎉 APP LIVE\n\n📱 ${result.projectName}\n🌐 ${deployment.url}\n❤️ Health check: HTTP ${deployment.health.status}\n🌿 ${result.branch}${admin}\n\nThe app is deployed and responding.`);
    } catch (error) { const text = error instanceof Error ? error.message : 'Autonomous build/deployment failed.'; const current = this.builds.get(telegramId); this.builds.set(telegramId, { ...(current ?? { updatedAt: Date.now() }), status: 'failed', message: text, updatedAt: Date.now() }); await this.api.editMessage(chatId, statusMessageId, `❌ AUTONOMOUS DEPLOYMENT FAILED\n\n${text}`); }
  }

  private async handleDeploy(chatId: string, telegramId: string, branch: string): Promise<void> {
    const saved = this.builds.get(telegramId); const selectedBranch = branch || saved?.branch; const projectName = saved?.projectName || 'Telegram Generated App'; const prompt = saved?.prompt || '';
    ...
    const status = await this.api.sendMessage(chatId, `🚀 Deploying ${selectedBranch} to Vercel…\n\n⏳ Health checking production before reporting success.`); const statusMessageId = (status as { message_id: number }).message_id;
    try { const deployment = await this.appBuilder.deployExistingBranch(process.env.GITHUB_OWNER, process.env.GITHUB_REPOSITORY, selectedBranch, projectName, prompt); const admin = deployment.adminCredentials ? `\n\n🔐 ADMIN LOGIN\nEmail: ${deployment.adminCredentials.email}\nPassword: ${deployment.adminCredentials.password}\n\n⚠️ Save these credentials securely.` : ''; this.builds.set(telegramId, { status: 'completed', message: `🌐 ${deployment.url}\n❤️ Health check: HTTP ${deployment.health.status}`, updatedAt: Date.now(), branch: selectedBranch, projectName, prompt, url: deployment.url, adminEmail: deployment.adminCredentials?.email, adminPassword: deployment.adminCredentials?.password }); await this.api.editMessage(chatId, statusMessageId, `🎉 DEPLOYMENT COMPLETE\n\n📱 ${projectName}\n🌐 ${deployment.url}\n❤️ Health check: HTTP ${deployment.health.status}\n🌿 ${selectedBranch}${admin}`); } catch (error) { const text = error instanceof Error ? error.message : 'Deployment failed.'; await this.api.editMessage(chatId, statusMessageId, `❌ DEPLOYMENT FAILED\n\n${text}`); }
  }
