...
export class TelegramHandlers {
  ...
  private async handleBuild(chatId: string, telegramId: string, prompt: string): Promise<void> {
    ...
    try { const result = await this.appBuilder.generate(prompt, process.env.GITHUB_OWNER, process.env.GITHUB_REPOSITORY, process.env.GITHUB_TOKEN, false); this.builds.set(telegramId, { status: 'completed', message: `${result.projectName}\nBranch: ${result.branch}\nFiles: ${result.files.length}`, updatedAt: Date.now() }); await this.api.editMessage(chatId, statusMessageId, `✅ APP GENERATED\n\n📱 ${result.projectName}\n\n${result.summary}\n\n📦 ${result.files.length} files\n🌿 Branch: ${result.branch}\n🐙 ${result.repositoryUrl}/tree/${result.branch}/generated/${result.slug}\n\n🚀 Run /deploy ${result.branch} to deploy it.`); } catch (error) { const text = error instanceof Error ? error.message : 'App build failed.'; this.builds.set(telegramId, { status: 'failed', message: text, updatedAt: Date.now() }); await this.api.editMessage(chatId, statusMessageId, `❌ APP BUILD FAILED\n\n${text}`); }
  }

  private async handleDeploy(chatId: string, telegramId: string, branch: string): Promise<void> {
    if (!branch) { await this.api.sendMessage(chatId, '🚀 Usage: /deploy <generated-branch>'); return; }
    ...
    const status = await this.api.sendMessage(chatId, '🚀 Deploying your app to Vercel…\n\n⏳ Waiting for Vercel build and deployment.'); const statusMessageId = (status as { message_id: number }).message_id;
    try { const result = await this.appBuilder.generate(`Deploy the existing generated project from branch ${branch}`, process.env.GITHUB_OWNER, process.env.GITHUB_REPOSITORY, process.env.GITHUB_TOKEN, true); this.builds.set(telegramId, { status: 'completed', message: `${result.projectName}\n${result.deployment?.url ?? 'deployment pending'}`, updatedAt: Date.now() }); await this.api.editMessage(chatId, statusMessageId, `🎉 DEPLOYMENT COMPLETE\n\n📱 ${result.projectName}\n🌐 ${result.deployment?.url ?? 'No deployment URL returned'}\n🌿 ${result.branch}\n\nYour app is now deployed.`); } catch (error) { const text = error instanceof Error ? error.message : 'Deployment failed.'; await this.api.editMessage(chatId, statusMessageId, `❌ DEPLOYMENT FAILED\n\n${text}`); }
  }
