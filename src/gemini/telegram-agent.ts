...
import type { AIProvider, AIProviderContent } from '../ai/provider.js';
...
export class TelegramGeminiAgent {
  ...
  constructor(client: AIProvider, dispatcher: GeminiToolDispatcher, storage?: S3MediaStorage) { this.loop = new GeminiAgentLoop(client, dispatcher); this.storage = storage; }
  ...
  async run(input: TelegramAgentInput) {
    ...
    const contents: AIProviderContent[] = history.map(message => ({ role: message.role === 'assistant' ? 'model' : 'user', parts: [{ text: `${message.role}: ${message.text}` }] }));
    if (this.storage) for (const asset of assets) { if (!asset.storageKey) continue; const bytes = await this.storage.get(asset.storageKey); if (bytes.byteLength > MAX_INLINE_MEDIA_BYTES) continue; contents.push({ role: 'user', parts: [{ inlineData: { mimeType: asset.mimeType, data: Buffer.from(bytes).toString('base64') } }] }); }
    contents.push({ role: 'user', parts: [{ text: `${active}${attached}\n\nCurrent request:\n${input.userPrompt}` }] });
    return this.loop.run({ systemPrompt: 'You are the production Telegram AI media assistant. Use registered tools for real operations, never claim success without a successful tool result, preserve user intent, inspect supplied multimodal media, maintain conversation context, and return tool results before the final Telegram response.', userPrompt: input.userPrompt, contents, context, maxToolRounds: 5 });
  }
}
