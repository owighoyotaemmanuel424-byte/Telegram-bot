...
export type BuildResult = { projectName: string; slug: string; summary: string; files: GeneratedFile[]; branch: string; repositoryUrl: string };
...
export class AppBuilderService {
  ...
  async generate(prompt: string, owner: string, repo: string, token: string): Promise<BuildResult> {
    const response = await this.gemini.generateContent({
      systemInstruction: `You are a senior full-stack engineer generating a small, runnable web application from a Telegram user's request. Return ONLY valid JSON with this shape: {"projectName":"...","summary":"...","files":[{"path":"...","content":"..."}]}. Use Vite + React + TypeScript + Tailwind-compatible plain CSS unless the request clearly needs another stack. The generated project must be self-contained, runnable with npm install && npm run build, and must not contain secrets. Keep it MVP-sized: at most ${MAX_FILES} files and each file under ${MAX_FILE_BYTES} bytes. Include package.json, index.html, src/main.tsx and required source files. Do not use markdown fences.`,
    ...
    const files = parsed.files
      ...
      .slice(0, MAX_FILES)
      .map(file => ({ path: file.path.replace(/^\/+/, '').replace(/\.\.(\/|\\)/g, ''), content: file.content }))
      .filter(file => file.path && !file.path.startsWith('.env') && Buffer.byteLength(file.content, 'utf8') <= MAX_FILE_BYTES);
  ...
  private async publish(owner: string, repo: string, token: string, branch: string, slug: string, files: GeneratedFile[], message: string): Promise<void> {
    ...
    for (const file of files) {
      ...
      await this.github(token, `/repos/${owner}/${repo}/contents/${path}`, { method: 'PUT', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ message: `${message}: ${file.path}`, content: Buffer.from(file.content, 'utf8').toString('base64'), branch }) });
    }
  }
}
