...
function createAdminCredentials(slug: string): AdminCredentials {
  return {
    ...
    password: `Adm-${randomBytes(12).toString('base64url')}`,
  };
}
...
export class AppBuilderService {
  ...
  async generate(prompt: string, owner: string, repo: string, token: string): Promise<BuildResult> {
    ...
    const files = parsed.files
      .filter((file): file is { path: string; content: string } => Boolean(file && typeof file === 'object' && typeof (file as any).path === 'string' && typeof (file as any).content === 'string'))
      .slice(0, MAX_FILES)
      .map(file => sanitizeFile(file.path, file.content))
    ...
    return {
      projectName,
      slug,
      summary: typeof parsed.summary === 'string' ? parsed.summary : 'Generated application',
      files,
      branch,
  ...
  async deployExistingBranch(owner: string, repo: string, branch: string, projectName: string, prompt = ''): Promise<NonNullable<BuildResult['deployment']>> {
    ...
    if (wantsAdmin(prompt)) {
      ...
      await this.deployer.setProductionEnv(first.projectId, {
      ...
      if (!healthConfigured.ok) {
        try {
          await this.deployer.rollback(slug, previous?.id ?? first.id);
        } catch {
          // Rollback is best-effort; the deployment error is still reported.
        }
    ...
    if (!health.ok) {
      ...
      if (previous) {
        try {
          await this.deployer.rollback(slug, previous.id);
          rolledBack = true;
        } catch {
...
`;
  ...
  private async repairFiles(errorLog: string, slug: string): Promise<GeneratedFile[]> {
    const response = await this.gemini.generateContent({
      systemInstruction: `You repair a generated Vite React TypeScript app. Given a build log, return ONLY JSON: {"files":[{"path":"relative/path","content":"complete replacement file"}]}. Only return files that must change. Never return .env files, secrets, or paths outside the project. The project directory is generated/${slug}.`,
    ...
    return parsed.files
      .filter((file): file is GeneratedFile => Boolean(file && typeof file === 'object' && typeof (file as any).path === 'string' && typeof (file as any).content === 'string'))
      .slice(0, 10)
      .map(file => sanitizeFile(file.path, file.content))
      .filter((file): file is GeneratedFile => Boolean(file));
  }
  ...
  private async waitForBuild(owner: string, repo: string, token: string, branch: string, waitForNew: boolean): Promise<{ id: number; conclusion: string | null }> {
    ...
    if (waitForNew) {
      ...
      baseline = Math.max(0, ...(runs.workflow_runs ?? []).map((run: any) => Number(run.id) || 0));
    }

    while (Date.now() < deadline) {
      const runs = await this.github(token, `/repos/${owner}/${repo}/actions/runs?branch=${encodeURIComponent(branch)}&per_page=10`);
      const candidate = (runs.workflow_runs ?? [])
        .filter((run: any) => run.name === 'Generated App Build' && Number(run.id) > baseline)
        .sort((a: any, b: any) => Number(b.id) - Number(a.id))[0];

      if (candidate?.status === 'completed') {
        return { id: Number(candidate.id), conclusion: candidate.conclusion };
      }
  ...
  private async getRunLogs(owner: string, repo: string, token: string, runId: number): Promise<string> {
    ...
    for (const job of jobs.jobs ?? []) {
      try {
        logs.push(await this.githubText(token, `/repos/${owner}/${repo}/actions/jobs/${job.id}/logs`));
      } catch {
        logs.push(`Job ${job.name} failed without readable logs.`);
      }
    }
  ...
  private async githubText(token: string, path: string): Promise<string> {
    ...
    const body = await response.text();
    if (!response.ok) throw new Error(`GitHub logs ${response.status}: ${body.slice(0, 500)}`);
    return body;
  }
  ...
  private async publish(owner: string, repo: string, token: string, branch: string, slug: string, files: GeneratedFile[], message: string): Promise<void> {
    ...
    await this.github(token, `/repos/${owner}/${repo}/git/refs`, {
      method: 'POST',
    ...
    for (const file of files) {
      const path = `generated/${slug}/${file.path}`;
      await this.github(token, `/repos/${owner}/${repo}/contents/${path}`, {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
  ...
  private async updateFiles(owner: string, repo: string, token: string, branch: string, slug: string, files: GeneratedFile[], message: string): Promise<void> {
    for (const file of files) {
      const path = `generated/${slug}/${file.path}`;
      const existing = await this.github(token, `/repos/${owner}/${repo}/contents/${path}?ref=${encodeURIComponent(branch)}`);
      await this.github(token, `/repos/${owner}/${repo}/contents/${path}`, {
        method: 'PUT',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
